...
const SEVERITY_STYLES: Record<string, { border: string; label: string; icon: React.ComponentType<{ className?: string }> }> = {
    ...
    <div className="border-border my-2 overflow-hidden rounded-lg border">
      <div className="flex flex-col divide-y divide-border">
        {data.events.slice(0, 15).map((event) => {
          ...
          return (
            <div className={`border-l-2 pl-3 pr-2 py-2 ${style.border}`} key={event.id}>
              <div className="flex items-start justify-between gap-2">
                <div className="min-w-0 flex-1">
                  <div className="flex items-center gap-1.5">
                    <Icon className="size-3.5 shrink-0" />
                    <span className="text-xs font-medium leading-tight">{event.message}</span>
                  </div>
                ...
                <div className="flex shrink-0 items-center gap-1">
                  {event.hasMedia && (
                    <span className="text-muted-foreground/50 flex items-center gap-0.5" title={`${event.mediaCount} attachment(s)`}>
